...
class Search
  ...
  def initialize(search_requirements)
    ...
    @statistics = calculate_statistics(@search_rules, @sort_rules).to_h
  end
  ...
  def calculate_statistics(search_rules, sort_rules)
    Statistics.new(filtering_list_cars, search_rules, sort_rules)
  end
